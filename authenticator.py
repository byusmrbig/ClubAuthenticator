...
class authenticator:
	...
	def cardStatusCheck(self, sanitizedCardID):	# checks the membership status of the swiping student
	...
	def main(self):	# the main function
		...
		while(1):	# run access input indefinitely
			...
			swipeResult = self.cardStatusCheck(cleanedSwipeInput)	# determine club status of the person swipping their card
			self.swipeLog(cleanedSwipeInput, swipeResult)	# log the card swipe
			if swipeResult == PAY_DUES:	# if the user needs to pay their dues
				# signal the LED to red for two seconds
				# break, restarting the while loop
				continue
			elif swipeResult == AGREEMENT_NEEDED:	# if the user needs to sign the agreement
				# signal the LED to blue for 2 seconds
				# break, restarting the while loop
				continue
			elif swipeResult == FULL_MEMBER:
				# signal the LED to green for 2 seconds
				# break, restarting the while loop
				continue
